...
:
  ...
  m_CheckButton("Display value on scale widgets", FALSE),
  ...
  m_Button_Quit("Quit")
{
  ...
  m_Separator.set_margin_top(100);
...
/*
changes the values of scales when adj val1 changed
*/
void ExampleWindow::on_adjustment1_value_changed()
{

  const double val = m_adjustment_digits->get_value();
  m_VScale.set_digits(static_cast<int>(val));
  m_HScale.set_digits(static_cast<int>(val));
}

void ExampleWindow::on_adjustment2_value_changed() const {
